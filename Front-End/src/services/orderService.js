  ...
  try {
    ...
    for (const localOrder of orders) {
      try {
        ...
        console.log(`Successfully migrated order ${localOrder.orderNumber}:`, result);
        ...
        console.error(`Failed to migrate order ${localOrder.orderNumber}:`, err);
        failedOrders.push({
          orderNumber: localOrder.orderNumber,
          error: err.message
        });
    ...
    if (migratedOrders.length > 0 && window.confirm('Hapus pesanan yang sudah dimigrasi dari localStorage?')) {
      // Filter pesanan yang gagal dimigrasi
      const remainingOrders = orders.filter(order => 
        !migratedOrders.some(migrated => migrated.orderNumber === order.orderNumber)
      );
...
export const getOrderByNumber = async (token, orderNumber) => {
  try {
    ...
    console.log(`Fetching order by number: ${orderNumber} from endpoint: ${API_URL}/orders/by-number/${orderNumber}/`);
    
    if (!token) {
      console.warn("No authentication token provided, user may not be logged in");
    ...
    if (!orderNumber.startsWith('ORD') || orderNumber.includes('-')) {
      ...
      console.log('Order number does not match backend format, checking session storage');
      ...
      if (sessionOrder) {
        try {
          ...
          if (parsedOrder.orderNumber === orderNumber && parsedOrder.order_number) {
            // We found the order in session storage and it has the API order number
            apiOrderNumber = parsedOrder.order_number;
            console.log(`Found matching API order number in session: ${apiOrderNumber}`);
          }
        } catch (e) {
    ...
    const response = await fetch(`${API_URL}/orders/by-number/${apiOrderNumber}/`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
...
export const confirmPayment = async (orderNumber) => {
  try {
    ...
    console.log(`[DEBUG] Starting confirmPayment for order: ${orderNumber}, token exists: ${!!token}`);
    ...
    if (sessionOrder) {
      try {
        ...
        if (parsedOrder.id || 
           (parsedOrder.order_number && parsedOrder.order_number === orderNumber) ||
           (parsedOrder.orderNumber && parsedOrder.orderNumber === orderNumber)) {
          
          // Use the ID directly if available
          orderId = parsedOrder.id;
          console.log(`[DEBUG] Using order ID from session storage: ${orderId}`);
        } else {
          console.log(`[DEBUG] Order in session doesn't match our criteria. session order_number: ${parsedOrder.order_number}, orderNumber: ${parsedOrder.orderNumber}, target: ${orderNumber}`);
        }
      } catch (e) {
    ...
    if (!orderId) {
      console.log('[DEBUG] No order ID from session, attempting to fetch from API');
      try {
        ...
        console.log(`[DEBUG] Fetching order details from API for number: ${orderNumber}`);
        ...
        orderId = order.id;
        console.log(`[DEBUG] Successfully fetched order ID from API: ${orderId}`);
      } catch (error) {
        ...
        if (!token || error.message.includes('not found')) {
          console.log('[DEBUG] Creating simulated payment success for guest user or non-existent order');
          ...
          console.log('[DEBUG] Returning simulated response:', simulatedResponse);
    ...
    if (!orderId) {
      console.error(`[DEBUG] Failed to find order ID for number: ${orderNumber}`);
      throw new Error(`Tidak dapat menemukan ID pesanan untuk nomor: ${orderNumber}`);
    }
    ...
    console.log(`[DEBUG] Processing payment for order ID: ${orderId}`, paymentData);
    ...
    console.log(`[DEBUG] Payment confirmation response status: ${response.status}`);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error(`[DEBUG] Error response body: ${errorText}`);
      throw new Error(`Gagal mengkonfirmasi pembayaran: ${response.status} - ${errorText}`);
    }
    ...
    try {
      ...
      console.error('[DEBUG] Failed to parse payment response as JSON:', jsonError);
...
export const getUserPurchaseHistory = async (token, userId) => {
  try {
    ...
    const response = await fetch(`${API_URL}/orders/user/${userId}/completed/`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`,
    ...
    if (data && data.length > 0 && data[0].items && data[0].items.length > 0) {
      console.log('Sample order item structure:', JSON.stringify(data[0].items[0], null, 2));
      console.log('Has product_detail?', !!data[0].items[0].product_detail);
      if (data[0].items[0].product_detail) {
        console.log('Product detail fields:', Object.keys(data[0].items[0].product_detail));
      }
    }
    ...
    for (const order of data) {
      console.log(`Processing order ${order.order_number} with ${order.items?.length || 0} items`);
      ...
      for (const item of order.items) {
        ...
        const productId = item.product || (item.product_detail && item.product_detail.id);
        ...
        if (!purchaseHistory[productId]) {
          ...
          console.log(`Creating new product history entry for ${productName} (ID: ${productId})`);
    ...
    if (productsWithoutDetails.length > 0) {
      console.log(`Fetching details for ${productsWithoutDetails.length} products without complete info`);
      ...
      for (const productId of productsWithoutDetails) {
        ...
        if (productDetails) {
          ...
          console.log(`Updated product ${productId} with name: ${productDetails.name} and image: ${productDetails.image}`);
        }
      }
    }
    ...
    console.log(`Processed ${sortedHistory.length} unique products in purchase history`);
