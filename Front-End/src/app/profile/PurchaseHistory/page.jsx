...
export default function PurchaseHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchPurchaseHistory = async () => {
      ...
      try {
        console.log("Fetching purchase history with token and userId:", !!token, userId);
        ...
        if (history.length === 0) {
          console.log("No purchase history found - this could be because you have no completed orders or there's an issue with the API");
        }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {purchaseHistory.length > 0 ? (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {purchaseHistory.map((product) => (
            <div key={product.id} className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center mb-4">
                <div className="mr-4">
                  ...
                  <p className="text-gray-700">
                    <span className="text-green-600 font-medium">Dibeli {product.purchase_count} kali</span>
                  </p>
                </div>
              </div>
              ...
              <h3 className="font-medium text-gray-800 mt-4 mb-2">Riwayat Pembelian:</h3>
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {product.orders.map((order, index) => (
                  <div key={index} className="border-b pb-2">
                    <div className="flex justify-between">
                      <span className="text-sm">Order #{order.order_number}</span>
                      <span className="text-sm">{formatDate(order.purchase_date)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>{order.quantity}x {order.size || 'Regular'}</span>
                      <span className="font-medium">{formatRupiah(order.price * order.quantity)}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="text-center py-8">
          <p className="text-gray-600 mb-4">Anda belum memiliki riwayat pembelian</p>
          <button
